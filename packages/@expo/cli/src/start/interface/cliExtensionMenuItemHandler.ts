...
export const cliExtensionMenuItemHandler = async (
  ...
  // If apps.length > 1, we could prompt the user to select an app, but for simplicity, we'll just use the first one.
  // In the future, we could allow the plugin to specify whether it needs to prompt for an app selection.
  let app = apps[0];
